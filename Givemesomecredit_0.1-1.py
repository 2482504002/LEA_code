...
juleicaaa=[]
for i in range (1,10):
   X_scale=X_scaled[:, 0:i]
   Xt_scaled = scaler.fit_transform(X_test)
   Xt_scale=Xt_scaled[:, 0:i]
   X_scale1=X_scaled[:, i:10]
   # 使用K-Means算法进行聚类
   ...
   juleicaaa.append(kkk/len(y_train))
   ...
   w_c = np.random.rand(i).reshape(i,1)
   w_s = np.random.rand(10-i).reshape(10-i,1)
   ...
   for ii in range(num_iterations):
      ...
      l_a=(tmp_suma/(-112500)).reshape(i,1)
   ...
   for ie in range(num_iterations):
      ...
      l_b=(tmp_sumb/(-112500)).reshape(10-i,1)
      ...
      if ie % 100 == 0:
         #costs.append(cost)
         print("Cost after iteration %i:" % (ie))
