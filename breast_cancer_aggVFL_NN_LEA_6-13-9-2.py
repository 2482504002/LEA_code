...
def load_data():
   ...
   # 将数据集分为训练集和测试集
    ...
    X_train_c2=scaler.fit_transform(X_train_c2)

    X_test_c=scaler.fit_transform(X_test_c)
    X_test_c1=scaler.fit_transform(X_test_c1)
    X_test_c2=scaler.fit_transform(X_test_c2)
    ...
    X_train_c2=torch.from_numpy(X_train_c2.astype(np.float32))
    X_test_c=torch.from_numpy(X_test_c.astype(np.float32))
    X_test_c1=torch.from_numpy(X_test_c1.astype(np.float32))
    X_test_c2=torch.from_numpy(X_test_c2.astype(np.float32))
    ...
    return X_train_c, X_test_c, y_train_c, y_test_c, X_train_s, X_test_s, y_train_s, y_test_s,X_train_c1, X_test_c1,X_train_c2, X_test_c2
...
X_train_c, X_test_c, y_train_c, y_test_c, X_train_s, X_test_s, y_train_s, y_test_s,X_train_c1, X_test_c1,X_train_c2, X_test_c2=load_data()
# 创建逻辑回归模型c
criterion=torch.nn.BCELoss()
...
optimizer_c00=torch.optim.SGD(model_c00.parameters(),lr=0.01)
...
with torch.no_grad():
    ...
    y_prediction00=model_c00(X_test_c2)
