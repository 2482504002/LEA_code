...
class bottle_Net(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(10, 20, 3) # 输入通道数10，输出通道数20，核的大小3
        # 下面的全连接层Linear的第一个参数指输入通道数，第二个参数指输出通道数
        self.fc1 = nn.Linear(20*3*10, 500) # 输入通道数是2000，输出通道数是500
        self.fc2 = nn.Linear(500, 10) # 输入通道数是500，输出通道数是10，即10分类
    def forward(self,x):
        in_size = x.size(0) # 在本例中in_size=512，也就是BATCH_SIZE的值。输入的x可以看成是512*1*28*28的张量。
        out = self.conv1(x) # batch*1*28*28 -> batch*10*24*24（28x28的图像经过一次核为5x5的卷积，输出变为24x24）
...
bottle_layer_c=[cv1,cv2,cv3,cv4]
...
b0=np.where(k_labels==l0)
kk_data0=np.array([k_data_c[idx] for idx in b0[0]])
kk_labels0=np.array([k_labels[idx] for idx in b0[0]])
...
b4=np.where(k_labels==l4)
kk_data4=np.array([k_data_c[idx] for idx in b4[0]])
kk_labels4=np.array([k_labels[idx] for idx in b4[0]])

kk_data=np.vstack((kk_data0, kk_data1, kk_data2, kk_data3, kk_data4))
kk_labels=np.hstack((kk_labels0, kk_labels1, kk_labels2, kk_labels3, kk_labels4))
len1=len(kk_labels0)
len2=len(kk_labels1)
len3=len(kk_labels2)
len4=len(kk_labels3)
        ...
        for x in range(class_num):
            for y in range(class_num):
                if i==j or i==x or i==y or j==x or j==y or x==y:
                    continue
                simulate_labels = np.hstack((np.full(len1, i), np.full(len2, j), np.full(len3, x), np.full(len4, y), np.full(len5, 10-x-y-i-j)))
...
for p in range(net_num):
    ...
    cv3=np.hstack((c_layers["fc1.weight"].cpu(), c_layers["fc1.bias"].reshape(500, 1).cpu()))
    cv4=np.hstack((c_layers["fc2.weight"].cpu(), c_layers["fc2.bias"].reshape(10, 1).cpu()))
    bottle_layer_s=[cv1,cv2,cv3,cv4]
...
small=simulate_labels_list[x1][0]
big=simulate_labels_list[x1][-1]
mid=3-small-big
print(f"Get attack model:{small}{123}{big}")
