...
for ii in range(10):
   feature_indices_c=feature_indices[:(ii+1)*3]
   ...
   with torch.no_grad():
      ...
      y_prediction1=torch.sigmoid(y_prediction1)
      y_prediction2=model_c2(X_test_c)
      y_prediction2=torch.sigmoid(y_prediction2)
      #V=torch.cat((y_prediction1,y_prediction2), dim=1)
      
      y_pred_class1=y_prediction1.round()
      y_pred_class2=y_prediction2.round()
      accuracy1=(y_pred_class1.eq(y_test_c).sum())/float(y_test_c.shape[0])#y_test_c=y_test_s
      accuracy2=(y_pred_class2.eq(y_test_c).sum())/float(y_test_c.shape[0])#y_test_c=y_test_s
   ...
   b4,w4,cc4,c4=compare_layers_cosine_similarity(bottle_layer,bottle_layer2)
   print(f"c1: {b3,w3,cc3,c3}, c2: {b4,w4,cc4,c4}")'''
   c1=cosine_similarity(detac, deta1,dim=1).sum()/len(detac)
   c2=cosine_similarity(detac, deta2,dim=1).sum()/len(detac)
