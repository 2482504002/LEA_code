...
from MyNet import CustomResNet18,bottle_ResNet18,top_ResNet18
...
def simu_eval(model,labels_of_interest):
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            x = model.resnet18.layer4(outputs1)

            x = model.resnet18.avgpool(x)
            x = torch.flatten(x, 1)
            outputs1 = model.resnet18.fc(x)
...
def spliteval(custom1_resnet18, custom2_resnet18,top_resnet18,labels_of_interest):
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            V=outputs1+outputs2
            x = custom1_resnet18.resnet18.layer4(V)

            x = custom1_resnet18.resnet18.avgpool(x)
            x = torch.flatten(x, 1)
            y_prediction = custom1_resnet18.resnet18.fc(x)
            #y_prediction =server_resnet18(V)
...
custom1_resnet18 = bottle_ResNet18()
simu_bottle=deepcopy(custom1_resnet18)
simu_models=[]
grad=[0]*simu_num
custom2_resnet18 = bottle_ResNet18()
server_resnet18=deepcopy(custom1_resnet18)

custom1_resnet18.to(device)
custom2_resnet18.to(device)
server_resnet18.to(device)
...
optimizer3 = optim.SGD(server_resnet18.parameters(), lr=0.001, momentum=0.9)
...
spliteval(custom1_resnet18,custom2_resnet18,server_resnet18,labels_of_interest)
...
for epoch in range(1): 
    for n in range(simu_num):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            x = simu_model.resnet18.layer4(outputs1)

            x = simu_model.resnet18.avgpool(x)
            x = torch.flatten(x, 1)
            outputs1 = simu_model.resnet18.fc(x)
