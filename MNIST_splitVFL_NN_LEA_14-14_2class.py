...
def get_two_random_data_targets(k_data, k_labels, batch_size,i):
    k_data_list1,k_data_list2, k_labels_list=[],[], []
    selected_numbers = random.sample([x for x in range(len(k_data))], 300)
    selected_numbers = [x for x in range(300*i,300*(i+1))]
    for i in selected_numbers:
        k_data_c=k_data[i][:14,:]
        k_data_s=k_data[i][14:,:]
        k_data_list1.append(k_data_c)
        k_data_list2.append(k_data_s)
        k_labels_list.append(k_labels[i])
    data1 = np.stack(k_data_list1)
    data2 = np.stack(k_data_list2)
    data1 = np.squeeze(data1).reshape(batch_size, 392)
    data2 = np.squeeze(data2).reshape(batch_size, 392)
    target=np.array(k_labels_list)
...
cc_layers=deepcopy(net_c.state_dict())
...
for e in range(epoch):
    ...
    for i in range(m//batch_size):
        data,data1,target1 = get_two_random_data_targets(k_data, k_labels, batch_size,i)
        ...
        y_prediction1=net_c(data)
        y_prediction2=net_s(data1)
        V=torch.cat((y_prediction1,y_prediction2), dim=1)
        y_prediction =net_t(V)
        '''
        dy=target1-y_prediction

        # 计算梯度
        tmp_xa=torch.matmul(X_train_c.T,dy)
        tmp_xb=torch.matmul(X_train_s.T,dy)
        tmp_xt=torch.matmul(V.T,dy)
        tmp_xaa=torch.matmul(tmp_xa,model_t.layer2.weight.data[:,:20])
        tmp_xbb=torch.matmul(tmp_xb,model_t.layer2.weight.data[:,20:])
        ...
        loss = F.nll_loss(y_hat, target1)
        loss.backward()
        if e== 2 and i==20:
             
            in_size = data.size(0) # 在本例中in_size=512，也就是BATCH_SIZE的值。输入的x可以看成是512*1*28*28的张量。
            out = net_c.conv1(data) # batch*1*28*28 -> batch*10*24*24（28x28的图像经过一次核为5x5的卷积，输出变为24x24）
            out = F.relu(out) # batch*10*24*24（激活函数ReLU不改变形状））
            out = F.max_pool2d(out, 2, 2) # batch*10*24*24 -> batch*10*12*12（2*2的池化层会减半）
            out = net_c.conv2(out) # batch*10*12*12 -> batch*20*10*10（再卷积一次，核的大小是3）
            out = F.relu(out) # batch*20*10*10
            out = out.view(in_size, -1) # batch*20*10*10 -> batch*2000（out的第二维是-1，说明是自动推算，本例中第二维是20*10*10）
            out_ni=np.linalg.pinv(out.cpu().detach().numpy())
            w1t=torch.matmul(net_c.fc1.weight.grad.data.cpu(),torch.from_numpy(out_ni))
        if e== 0:
