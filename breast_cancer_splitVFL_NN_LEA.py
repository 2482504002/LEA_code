...
class bottle_net(torch.nn.Module):
   def __init__(self,no_input_features):
      ...
      self.layer1=torch.nn.Linear(no_input_features,20)
      self.layer0=torch.nn.Linear(20,64)
      self.layer00=torch.nn.Linear(64,20)
      #self.layer2=torch.nn.Linear(20,1)
   def forward(self,x):
      y_predicted=self.layer1(x)
      y_predicted=self.layer0(y_predicted)
      y_predicted=self.layer00(y_predicted)
      #y_predicted=torch.sigmoid(self.layer2(y_predicted))
      return y_predicted
...
class simulate_net(torch.nn.Module):
   def __init__(self,no_input_features):
      ...
      self.layer0=torch.nn.Linear(20,64)
      self.layer00=torch.nn.Linear(64,20)
      self.layer2=torch.nn.Linear(20,256)
      self.layer3=torch.nn.Linear(256,126)
      self.layer4=torch.nn.Linear(126,64)
   ...
   def forward(self,x):
      ...
      y_predicted=self.layer5(y_predicted)
      #y_predicted=torch.sigmoid(self.layer2(y_predicted))
      y_predicted=torch.sigmoid(self.layer6(y_predicted))
      return y_predicted
...
class top_net(torch.nn.Module):
   def __init__(self,client_num):
      ...
      #self.layer1=torch.nn.Linear(no_input_features,20)
      self.layer2=torch.nn.Linear(client_num*20,256)
      self.layer3=torch.nn.Linear(256,126)
      self.layer4=torch.nn.Linear(126,64)
      self.layer5=torch.nn.Linear(64,10)
      self.layer6=torch.nn.Linear(10,1)
...
all_X_train=scaler1.fit_transform(X_train_c)
...
model_c1.layer0=deepcopy(model_c.layer0)
model_c1.layer00=deepcopy(model_c.layer00)
...
bottle_layer1 = model_c1.layer6
bottle_layer2 = model_c2.layer6
...
b3,w3,cc3,c3=compare_layers_cosine_similarity(bottle_layer1,bottle_layer2)
print(f"c1: {b1,w1,cc1,c1}")
print(f"c2: {b2,w2,cc2,c2}")
print(f"c3: {b3,w3,cc3,c3}")
'''
